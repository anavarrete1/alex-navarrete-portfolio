-- Query 1
-- Selects all players who are goalkeepers and anyone who has 'o' in their last name
USE section1_team04;
...
-- Query 3
-- This query grabs player information and whether they have scored 5 or 10 goals.
...
-- Query 4 
-- Combines the tables that show players, team, and league that they are currently in. 
USE section1_team4;
...
-- QUERY 5: This query has an arithmetic expression that will compute each team's overall points accumulated from wins, ties, and losses.
...
-- QUERY 6: This query will use the MAX group function to determine who the players are who have scored the most goals
SELECT CONCAT(First_Name, ' ', Last_Name) AS 'Player Full Name', MAX(Player_Goals) AS 'Top scorers'
FROM player
GROUP BY 1
ORDER BY 2 DESC;

-- QUERY 7: This query will show all the teams that have more trophies than the average team
SELECT Team_Name AS 'Team Name', Team_Trophies AS 'Number of Trophies'
FROM team
WHERE team_trophies > (
